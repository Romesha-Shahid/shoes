...
// import { useCart } from "@/components/Cart-Context"; // Assuming you're using CartContext here
...
//   const { items = [], updateQuantity } = useCart(); // Ensure items is always an array

//   const subtotal = items.reduce((total: number, item: { price: any; quantity: number; }) => {
...
import { useCart } from "../Context/CartContect"; // Assuming you're using CartContext here
import Image from "next/image";
import { PlusCircle, MinusCircle } from 'lucide-react';
import Link from "next/link";
// import { StaticImport } from "next/dist/shared/lib/get-img-props";
...
export default function CartPage() {
  ...
  const subtotal = items.reduce((total: number, item: { price: unknown; quantity: number; }) => {
    let price = item.price;
...
//     // Handle price correctly if it's a string or number
//     if (typeof price === 'string') {
//       price = parseFloat(price.replace(/[^0-9.]/g, '')); // Remove non-numeric characters
//     } else if (typeof price === 'number') {
//       price = parseFloat(price.toString()); // Ensure it's treated as a number
//     }
